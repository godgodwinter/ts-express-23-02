...
class redisProsesService {
    constructor(req) {
        ...
        this.proses_siswa_delete = async (siswa_id) => {
            const cacheKey = `STUDIV2_PROSES_SISWA_ID_${siswa_id}`;
            try {
                const cachedResult = await babengRedis_1.default.get(cacheKey);
                if (cachedResult) {
        ...
        this.proses_siswa_store = async (siswa_id) => {
            const cacheKey = `STUDIV2_PROSES_SISWA_ID_${siswa_id}`;
                ...
                for (const [index_mapel, mapel] of get_studi_v2_proses_aspek_detail.entries()) {
                    ...
                    for (const [index_soal, soal] of get_soal.entries()) {
                        ...
                        if (mapel.random_soal === "Aktif") {
                            ...
                            get_pj = await studi_v2_paketsoal_pilihanjawaban.scope('lessData').findAll({ where: { kode_soal: soal.kode_soal, studi_v2_paketsoal_soal_id: soal.studi_v2_paketsoal_soal_id, deleted_at: null }, order: [sequelize_1.Sequelize.literal('RAND()')] });
        ...
        this.proses_kelas_store = async (kelas_id) => {
        ...
        this.proses_kelas_delete = async (kelas_id) => {
            const siswa_Service = new siswa_v2_service_1.default(this.req);
            const response = await siswa_Service.siswaGetWhereKelas(kelas_id);
            for (const [index, item] of response.entries()) {
...
exports.default = redisProsesService;
